...
const INPUTS_CHECK_REGEXP_MAP: I_INPUTS_PROPS_MAP = {
	card: { res: v => ({ value: re(v)(/\b\d{1,16}\b/) && v, isCorrect: re(v)(/\b\d{16,17}\b/) }) },
	...
	cvv: { res: v => ({ value: re(v)(/\b\d{1,3}\b/) && v, isCorrect: re(v)(/\b\d{3,4}\b/) }) },
}

export const useValidate = () => {
	const [_checked_value, checkValue] = useState<hook_data>({ value: '', TYPE: 'card' }),
		{ TYPE, value: val } = _checked_value,
